...
defmodule Day do
  ...
  def neighbours(lookup, {x, y, z, w}) do
    neighbours =
      for xd <- -1..1 do
        for yd <- -1..1 do
          for zd <- -1..1 do
      ...
      |> List.flatten()
      |> Enum.reject(fn pos -> pos == {0, 0, 0, 0} end)
      |> Enum.map(fn {xd, yd, zd, wd} -> {x + xd, y + yd, z + zd, w + wd} end)
