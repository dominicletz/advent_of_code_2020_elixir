...
defmodule Day do
  def run() do
    [attributes, my_ticket, nearby] = File.read!("16.csv")
    |> String.split("\n\n", trim: true)

    attributes = String.split(attributes, "\n", trim: true)
      |> Enum.map(fn row ->
        [label, filter] = String.split(row, ": ")
        range = String.split(filter, " or ")
    ...
    my_ticket = String.split(my_ticket, "\n", trim: true)
    |> Enum.drop(1)
  ...
  def reduce(attributes, cols, ret) do
    unmatched = Enum.map(attributes, fn {label, range, _matches} ->
      matches = Enum.filter(cols, fn {_i, values} ->
    ...
    min = {_label, _range, candidates} = Enum.min(unmatched, fn {_la, _ra, matches_a}, {_lb, _rb, matches_b} -> length(matches_a) < length(matches_b) end)
